...
CREATE TABLE IF NOT EXISTS `health-care-gcp-project.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `health-care-gcp-project.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `health-care-gcp-project.silver_dataset.claims` c
